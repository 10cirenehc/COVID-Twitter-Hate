# -*- coding: utf-8 -*-
"""
Created on Wed Sep  2 13:00:44 2020
...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 27 10:05:34 2020
...
def optimal_model(df_in,label_in,path_in,params_in):
    my_rf = RandomForestClassifier(random_state = 666)
    ...
    print(best_params)
    my_rf_optimal = RandomForestClassifier(**best_params,random_state = 666)
    my_rf_optimal.fit(df_in,label_in)
    pickle.dump(my_rf_optimal,open(path_in + "model_2.pkl","wb"))
...
if __name__ == '__main__':
    ...
    df = df.loc[(df.expert == "entity_directed_hostility") | (df.expert == "entity_directed_criticism")]
    tweets = df.text
    ...
    my_pca_matrix = np.array(my_pca.fit_transform(sentiment_matrix))
    pickle.dump(my_pca,open(path_in + "pca.pkl","wb"))
    ...
    import pickle
    pickle.dump(X,open(path_in + "X_2.pkl","wb"))
    pickle.dump(y,open(path_in + "y_2.pkl","wb"))
    
    # print("Training...")
    # parameters = {'n_estimators':[290,295,300,305,310],'max_depth': [14,15,16]}
    # my_model = optimal_model(X,y,path_in,parameters)
